#!/usr/bin/env sh
# Set the package/workspace version in Cargo.toml AND the workspace-member entries in
...
awk -v v="$version" '
  ...
  END {
      if (!done) {
          print "could not find a package version in Cargo.toml" > "/dev/stderr"
          exit 1
      }
  }
' Cargo.toml > "$tmp"
...
echo "set package version = $version in Cargo.toml and Cargo.lock"
